...
const EditCourseModal = ({ isOpen, onClose, course, onSuccess }) => {
    ...
    const [formData, setFormData] = useState({ name: '', description: '', status: 'active', slug: '' });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const message = getApiErrorMessage(error, 'Ошибка обновления');
                ...
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#1A1A1C] rounded-2xl max-w-md w-full p-6 border border-white/10" onClick={e => e.stopPropagation()}>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea rows="3" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-purple-500 outline-none" />
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-400 mb-1">Короткий URL</label>
                            <input type="text" value={formData.slug} onChange={(e) => { setFormData({ ...formData, slug: e.target.value }); if (errors.slug) setErrors((prev) => ({ ...prev, slug: '' })); }} placeholder="Например: ремонт кухни" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-purple-500 outline-none" />
                            <p className="text-xs text-gray-500 mt-2">Итоговый короткий URL: <span className="text-gray-300">{slugifyPreview(formData.slug) || '—'}</span></p>
